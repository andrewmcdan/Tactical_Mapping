...
void setup() {
    ...
    if (!datagramManager.waitAvailableTimeout(30000)) { // if after 30 seconds nothing is received, assume there is no mesh
        ...
        while (!meshStatusRecvd) {
            ...
            if (recvBufLen == MESH_STATUS_LENGTH && recvBuf[0] == 0x0f && toAddr == RH_BROADCAST_ADDRESS) {
            ...
            if(!meshStatusRecvd){
                datagramManager.waitAvailableTimeout(60000); // waiting for another packet
...
void loop() {
    ...
    data[dataLength++] = 0x0f; // mesh status flag
